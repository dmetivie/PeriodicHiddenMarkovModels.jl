...
@testset "Comparing with HiddenMarkovModels in trivial case T = 1 (aperiodic)" begin
    dist = [Normal, Normal]
    K = length(dist)
    T = 1
    N = 5_000
    ...
    @test isapprox(collect.(Distributions.params.(hmm_fit.B)), collect.(Distributions.params.(ν)), rtol=5e-2)

    # HiddenMarkovModels.jl is a well tested package
    hmm_base = HiddenMarkovModels.HMM([1 / 2, 1 / 2], dropdims(Q, dims=ndims(Q)), dropdims(ν, dims=ndims(ν)))
    hmm_base_guess0 = HiddenMarkovModels.HMM([1 / 2, 1 / 2], dropdims(Q_guess, dims=ndims(Q_guess)), dropdims(ν_guess, dims=ndims(ν_guess)))
    hmm_base_fit, loglikelihood_evolution = baum_welch(hmm_base_guess0, y)
    @test hist["logtots"][end] ≈ loglikelihood_evolution[end] rtol = 1e-2
    @test hmm_fit.a ≈ hmm_base_fit.init
    @test hmm_fit.A[:, :, 1] ≈ hmm_base_fit.trans rtol = 1e-2
...
@testset "Periodic example" begin
    ...
    @test isapprox(collect.(Distributions.params.(hmm_fit.B)), collect.(Distributions.params.(ν)), rtol=10e-2)
end